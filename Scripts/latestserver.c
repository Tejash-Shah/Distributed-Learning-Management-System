...
//int student(sqlite3* database, int client_socket);
...
static const char* DATABASE_LOCATION = "/home/tejashshah/Downloads/207project/database/record.db";
...
//SERVER INFORMATION
static const int SERVER_PORT = 6364;
...
//-----------------------------------------------------------TEJASH'S VARIABLES-------------------------------------------------------//
  ...
  //----------------------------------------------------------------STUDENT LOGIN----------------------------------------------------//////
    ...
    if(strcmp(login_response,"Student")==0)
    {
  //------------------------------------------------------------OPEN DATABASE----------------------------------------------------------/////
  	...
  	R : asscogra();
    read(accept_value,&ans1,sizeof(ans1));
 		printf("Student opts for %s\n",&ans1);
 		if (strcmp(ans1,"Assignments")==0)
 		{
      courses();
 			read(accept_value,course_response,sizeof(course_response));
 			assignment();
 			Label : read(accept_value,&assignment_response,sizeof(assignment_response));
      ...
      printf("Course response=%s\n",course_response);
 			options();
 			read(accept_value,options_response,sizeof(options_response));
 			if(strcmp(options_response,"Files")==0)
 			{
 				file_name();
 				read(accept_value,&file_integer,sizeof(file_integer));
 			...
 			else if (strcmp(options_response,"Assignments")==0)
      {
        ...
        goto Label ;
 			...
 			{ 
 				write(accept_value, "Invalid Input, Try Again  ",26);
 				goto R;
 			}
 		...
 		{
 			grades();
 			read(accept_value,&grade_answer,sizeof(grade_answer));
  ...
  else if (strcmp(login_response,"Instructor")==0)
  {

  //server_address = CreateServerAddress();
  //server_socket = SocketFactory(server_address); // Create Socket Function
    system("lsof -i tcp| grep canvas_pr");
    printf("In Instructor Section\n");
    CheckDatabase();
  //while (true) {
      //int client_socket;
      pthread_t thread;
      //accept_value = accept(server_socket, (struct sockaddr *) &server_address, &addr_size);
      return_code = pthread_create(&thread, NULL, handle_client, (void *)accept_value);
...
void asscogra()
{
	write(accept_value,"Assignments  ",13);
	write(accept_value,"Courses  ",9);
 	write(accept_value,"Grades  ",8);
}
...
int assignment()
{
	write(accept_value,"WEEK1 ",6);
	write(accept_value,"WEEK2 ",6);
	write(accept_value,"WEEK3 ",6);
	write(accept_value,"WEEK4 ",6);
}

//----------------------------------------------------------------ASK COURSES ---------------------------------------------------------////
int courses ()
{ 
	write(accept_value,"207   ",6);
	write(accept_value,"209   ",6);
	//write(accept_value,"English  ",9);
}
//----------------------------------------------------------ASK WHICH SUBJECT GRADES ---------------------------------------------------------////
int grades()
{
	write(accept_value,"207Grades   ",12);
	write(accept_value,"209Grades   ",12);
	//write(accept_value,"English===Grades   ",16);
}
//----------------------------------------------------------------DISPLAY OPTIONS ---------------------------------------------------------////
int options()
{
	write(accept_value,"Files  ",7);
	write(accept_value,"Assignments  ",13);
}
//----------------------------------------------------------------DISPLAY FILES NAMES ---------------------------------------------------------////
int file_name()
{
	write(accept_value,"Chapterone ",11);
	write(accept_value,"Chaptertwo ",11);
	write(accept_value,"Chapterthree ",13);
	write(accept_value,"Chapterfour  ",13);
}
...
int tf(int sock, char *file_name)
	...
	{
	printf(file_name);
		if( (sentsize = send(accept_value, error_message , strlen(error_message), 0)) < 0 )
			{
			printf("send error");
			return -1;
			}
			...
			while( (readsize = read(f, send_buf, Receive_Buff)) > 0 )
				{
				if( (sentsize = send(accept_value, send_buf, readsize, 0))< readsize )
				{
				printf("send error");
				return (-1);
				}
...
//                    student(database, client_socket);
                }
                break;
            case 2:
                AboutUs((void *) &data);
...
void* admin(void* thread_args){
        ...
        char *add_user= "\n\nPlease enter user details to add a user: <role> <username> <password>:\n\n";
...
//        }
        ...
        while(true)
        {
            ...
            if(strcmp(client_buffer,"1")==0)
            {
                ...
                asprintf(&query, "insert into users (role, name, password) values ('%s','%s', '%s');", role ,name ,password);
            ...
            if(strcmp(client_buffer,"2")==0)
            {
                bzero(client_buffer,1500);
                printf("writing to the client\n");
                strcpy(client_buffer,"Enter the instructor name and subject: <instructor> <subject>:");
                n1=write(client_socket,client_buffer,strlen(client_buffer));
                if (n1 <= 0)
                {
                    perror("Server:ERROR writing to socket\n");
                ...
                asprintf(&query, "insert into Instructor_details (name, subject) values ('%s', '%s');",name ,password);
                sqlite3_prepare_v2(database,query, -1,&stmnt, &errmsg);
                rc = sqlite3_step(stmnt);
                if (rc == SQLITE_OK){
                    cout << "User inserted" << endl;
            ...
            if(strcmp(client_buffer,"3")==0)
            {
                bzero(client_buffer,1500);
                printf("writing to the client\n");
                strcpy(client_buffer,"Please enter instructor name: <instructor name>");
                n1=write(client_socket,client_buffer,strlen(client_buffer));
                if (n1 <= 0)
                {
                    perror("Server:ERROR writing to socket\n");
                ...
                bzero(stmnt,100);
                snprintf (query, 100, "SELECT subject from Instructor_details where name = '%s';", client_buffer);
                printf("here it is:%s",stmnt);
                bzero(client_buffer,1500);
                sqlite3_prepare_v2(database,query, -1,&stmnt, &errmsg);
                rc = sqlite3_step(stmnt);
                if (rc == SQLITE_OK){
//                    cout << "User deleted" << endl;
                    ...
                    strcat(client_buffer,errmsg);
                    strcat(client_buffer,"\n\n");
                    strcat(client_buffer,Sys_admin_options);
//                    sqlite3_free(errmsg);
                    //fprintf(stdout, "password %s info retrieved successfully\n",uname);
                }
//                else if(strcmp(db_call_result,"")==0){
//                    bzero(client_buffer,1500);
//                    printf("Error!!!!!");
//                    strcpy(client_buffer,"\n\n Unable to retrieve data. There is no subject related to this instructor \n");
//                    //strcat(client_buffer,&errmsg);
...
//                }
            ...
            {
                bzero(client_buffer,2500);
                bzero(stmnt,100);
                snprintf (query, 100, "SELECT name from users where role = 'Instructor';");
                printf("here it is:%s",stmnt);
//                bzero(db_call_result,2500);
                sqlite3_prepare_v2(database,query, -1,&stmnt, &errmsg);
                rc = sqlite3_step(stmnt);
                if (rc == SQLITE_OK){
//                    cout << "User deleted" << endl;
...
//                }
            ...
            if(strcmp(client_buffer,"5")==0)
            {
                bzero(client_buffer,1500);
                bzero(stmnt,100);
                snprintf (query, 100, "SELECT name from login where role = 'Student';");
                printf("here it is:%s",stmnt);
//                bzero(db_call_result,1500);
                sqlite3_prepare_v2(database,query, -1,&stmnt, &errmsg);
                rc = sqlite3_step(stmnt);
...
//                    bzero(client_buffer,1500);
//                    printf("Error!!!!!");
//                    strcpy(client_buffer,"\n\n Unable to retrieve data. There is no subject related to this Student \n");
//                    //strcat(client_buffer,&errmsg);
//                    //strcpy(client_buffer,db_call_result);
//                    strcat(client_buffer,"\n\n");
//                    strcat(client_buffer,Sys_admin_options);
...
//                }
            ...
            if(strcmp(client_buffer,"6")==0){
                bzero(client_buffer,1500);
                printf("writing to the clilent\n");

                strcpy(client_buffer,"\n\nPlease enter the name of teacher and subject to be deleted:<Instructor> <Subject>\n\n");
                n1=write(client_socket,client_buffer,strlen(client_buffer));
                if (n1 <= 0){
                    perror("Server:ERROR writing to socket\n");
                }
                ...
                asprintf(&query, "Delete from Instructor_details where name = '%s' and subject = '%s';",name,subject);
                sqlite3_prepare_v2(database,query, -1,&stmnt, &errmsg);
                rc = sqlite3_step(stmnt);
                if (rc == SQLITE_OK){
                    cout << "User deleted" << endl;
                ...
                printf("writing to the clilent\n");
                bzero(client_buffer,1500);
                strcpy(client_buffer,"User Deleted");
                strcat(client_buffer,"\n\n");
                strcat(client_buffer,Sys_admin_options);
                n1=write(client_socket,client_buffer,strlen(client_buffer));
                bzero(query,1500);
